...
6. **Inbound human-gate enforcement** (`phase:awaiting-inbound-approval`, REQ-SWARM-1). Inbound never auto-lands regardless of mode. *Verify:* new AC — a graphed inbound issue converges and parks; only `inbound-approved:land` advances it; the peer's answer reports the pending-approval state. The **executable end-to-end spec** of this whole inbound loop (arrive → sanitize → graph → work → park → answer-once → approve → land) is `crates/orchestrator/tests/federation_e2e.rs`.
